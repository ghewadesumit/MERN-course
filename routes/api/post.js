...
// @desc   Posting a comment on post
// @access Private
router.post(
  '/comment/:id',
  [auth, check('text', 'Text is required').not().isEmpty()],
  async (req, res, next) => {
    ...
    try {
      ...
      console.error(err.message);
      res.status(500).send('Server error for posting comment');
    }
  }
);

// @route  DELETE api/post/comment/:id/:comment_id
// @desc   Deleting a comment on post
// @access Private
router.delete('/comment/:id/:comment_id', auth, async (req, res, next) => {
  try {
    ...
    if (comment.user.toString() !== req.user.id)
      return res
        .status(401)
        .json({ msg: 'Unauthorized: You cannot delete other peoples comment' });
